...
##FIXME: Laha, it is really important that you state what the purpose of the
## vignette is. Then we can make sure it answers the questions.  I know this 
##was developed over time, but you could replace it with something that follows the outline in the box I made for our paper.  
##FIXME: here you want to describe what we are modeling for a response and what we are modeling for outcomes.
##I thought our first base model was around nutrition - but that seems to be gone.  This is the base model for the cobalt example then?

##FIXME - you need to describe the data more. What are you using, you should say something like.  For NHANES both systolic and diastolic blood pressure were measured. Each was measured two times and we chose to use the average of the two measurements.  Individuals with either value missing were excluded from the analysis.
...
##FIXME - I don't understand the sentence below - you have to explain in detail how/what you did. What does it mean filled with 0?
...
##FIXME: they are converted to factors? They were always categorical variables.


##FIXME; the code below mixes questionairre names - BPX_, but misses BPX with
no underscore. And then uses long names in jointQuery - please fix it so that 
it just uses one, and show probably with comments in the code how you know you go all the tables you need - also output in html looks like it has not run recently as there are no years reported in the table.
```{r pre}
...
## 3. Identify Confounders and Build Base Model
##but it isn't given -- you have to state the scientific question somewhere
Given the scientific question, we can identify a response or outcome of interest and covariates in the base models. 

##FIXME - I think this is based on cobalt - and so you can say that they adjusted for certain things and we will to.

##The statement below is not what I am seeing - the p-values for the interaction seem to be large and not significant.  We need to do a little work on the reporting of the model.  That is on me to figure out in the next week or so.
...
##FIXME: somehow this seems out of place.  Everything above, that shows
up in Section 3, seems like it should be removed. Section 3.2 should come before
this and then this section should come.
...
Education significantly impacts the outcome (diastolic) based on p-values. However, we can find the relative Sum of Sq for `DMDEDUC2` is  also considerably small.
...
ns_base_edu <- lm(formula = as.formula(ns_str_edu), base_df)

print_anova(anova(ns_base,ns_base_edu,test="LRT"))
sjPlot::tab_model(ns_base,ns_base_edu,
                  dv.labels = c("ns_base", "ns_base + Education"),
                  show.ci = FALSE,show.stat = TRUE,show.se = TRUE,p.style = "scientific", digits.p = 2)
```

Similar to education, years significantly impacts the outcome (diastolic) based on p-values. However, we can find the relative Sum of Sq for `year` is is also considerably small.
```{r ses_year}
...
lm_str <- 'DIASTOLIC ~ RIDAGEYR*RIAGENDR + BMXBMI + RIDRETH1+nDEDUC+years'
