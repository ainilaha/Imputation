...
nhanes_db <- dbConnect(RSQLite::SQLite(), "../../nhanes.sqlite")
# BPXDI2,BPXSY1
blood_pressure <- dbGetQuery(nhanes_db, "select SEQN, BPXDI1,BPXDI2,years from blood_pressure where BPXDI1 IS NOT NULL and BPXDI1 <> 0 and BPXDI2 IS NOT NULL and BPXDI2 <> 0")

dbDisconnect(nhanes_db)
blood_pressure$diastolic <- rowMeans(blood_pressure[, c('BPXDI1','BPXDI2')])


data <- merge(blood_pressure, data, 
by = c("SEQN",'years'))
...
mf <- reshape2::melt(pred_df,id.vars = c("Age", "Gender","BMI","DIASTOLIC"))
aw_age_lm <- ggplot(mf,
         aes(
           x = Age,
           y = DIASTOLIC
...
raw_age_ns <- ggplot(data,
  ...
  geom_point(data = ~ group_by(.x, RIDAGEYR, RIAGENDR,BMXBMI) |> sample_frac(0.5),
             alpha = 0.2, shape=1) +
    geom_smooth(aes(x = RIDAGEYR,y=ns_base$fitted.values,colour="Spline"),size=1.5)+
  xlab("Age (year)")+ylab("Diastolic (mm Hg)")+
   # scale_colour_manual(name="Model", values=c("#E69F00", "#56B4E9"))+ 
  theme_minimal()

ggpubr::ggarrange(aw_age_lm,raw_age_ns,
                           common.legend = TRUE,
                           labels = c('a)','b)'),legend='right') + scale_colour_manual(name="Model", values=c("#E69F00", "#56B4E9"))
...
## Binned plot for binned residuals against fitted value

```{r bmi_plot, echo=TRUE,warning=FALSE,message=FALSE,dpi = 200}
df_fitted <- list("Linear"=make_bins(x=lm_base$fitted.values,y=lm_base$residual,nbin=600),
                "Spline"=make_bins(x=ns_base$fitted.values,y=ns_base$residuals,nbin=600)
                )
plot_bins2(df_fitted,xlab="Fitted Value",ylab="Binned Residuals")
```
