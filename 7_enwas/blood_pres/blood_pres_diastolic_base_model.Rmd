...
bmi_res <- plot_bins2(df_bmi_res,xlab="BMI (kg/m²)")+ xlim(min(data$BMXBMI),max(data$BMXBMI))+ ylim(-8,6.5)
...
raw_age <- ggplot(data,
  ...
  geom_smooth() + labs(colour="Gender")+
...
The model with 10 knots has the lowest RSS. However, the relative change is almost negligible. 

```{r cv, echo=TRUE,warning=FALSE,message=FALSE}
RSS_avg <- c()
for(q in 5:20){
  # number of knots = q - 2
  knots <- quantile(unique(data$RIDAGEYR),1:(q-2)/(q-1))
  knots <- paste0(knots, collapse=', ' )
  ns_model <-
  paste0('BPXDI1 ~ ns(RIDAGEYR, knots = c(',knots,  '))* RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,75,by=10)))')
  ...
  for (year in years){
    cv_train <- data[data$years!= year,]
    cv_test <- data[data$years == year,]

    cv_ns_model <- lm(as.formula(ns_model), cv_train)
    
      pred_vals <- predict(cv_ns_model,cv_test)
      
      rss <- sum((pred_vals-cv_test$BPXDI1)^2)
      rss_vector <- c(rss_vector,rss)
    
  }
  RSS_avg <- c(RSS_avg,mean(rss_vector))
  

}

qplot(3:18,RSS_avg,geom=c("point", "line")) + xlab("Number of Knots") + ylab("Average RSS")
(max(RSS_avg) - min(RSS_avg))/mean(RSS_avg)
