...
library(latex2exp)
sacle_g_lm <- plot_bins(
  lm_base$fitted.values,
  sqrt(abs(scale(lm_base$residuals))),
  xlab = "fitted value",
  ylab = TeX(r'($\sqrt{|standardize\ residuals|}$)'),
  nbin = 200
)
sacle_g_ns <- plot_bins(
  ns_base$fitted.values,
  sqrt(abs(scale(ns_base$residuals))),
  xlab = "fitted value",
  ylab = TeX(r'($\sqrt{|standardize\ residuals|}$)'),
  title="spline",
  nbin = 200
...
ggarrange(sacle_g_lm, sacle_g_ns,common.legend = TRUE)
```
...
Both the outliers have extremely low Diastolic-Blood Pressure according to the distribution plotted above.
In addition, both cases are the age ranges (younger than 40 and older than 65), which are off from the linear regression. By removing these two outliers from the linear regression, the $R^2$ change from 0.03276 to 0.03277, which is a pretty minor impact because we can see more than these two outliers shown in the lower left corner of the plots because the linear regression fails to capture those points.

```{r Leverage1, echo=TRUE,warning=FALSE,message=FALSE,results = "asis",dpi = 200}
library(knitr)
kable(data[c(1738,3196),c('BPXDI1','RIAGENDR','BMXBMI','RIDAGEYR')])
