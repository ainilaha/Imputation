...
df_list_cook_iv <- list("Linear"=make_bins(x=invNorm(data$DR1TMOIS),y=cooks.distance(lm_mos),nbin=500),
                "Spline"=make_bins(x=invNorm(data$DR1TMOIS),y=cooks.distance(ns_mos),nbin=500)
                )

df_list_lev_iv <- list("Linear"=make_bins(x=invNorm(data$DR1TMOIS),y=hatvalues(lm_mos),nbin=500),
                "Spline"=make_bins(x=invNorm(data$DR1TMOIS),y=hatvalues(ns_mos),nbin=500)
                )
lev_g_iv <- plot_bins2(df_list_lev_iv,xlab="Inversed Moisture",ylab = "Leverage")
cook_g_iv <- plot_bins2(df_list_cook_iv,xlab="Inversed Moisture")
ggarrange(lev_g,cook_g,lev_g_iv,cook_g_iv,common.legend = TRUE)
```
