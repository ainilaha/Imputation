...
plot_bins<- function(x,y,xlab="bin avg",ylab="residuals",ncalss=100){
  if (ncalss==0)
    ncalss <- floor(sqrt(length(x)))
  
  df <-
  data.frame(
    y = y,
    breaks = cut(x,breaks = ncalss)
  )
  df <- df %>% 
  group_by(breaks) %>% 
...
g_lm_mos <- plot_bins(data$DR1TMOIS, lm_mos$residuals,500)
g_ns_mos <- plot_bins(data$DR1TMOIS, ns_mos$residuals,500)

ggarrange(g_lm_mos,g_ns_mos,common.legend = TRUE)

g_lm_mos <- plot_bins(invNorm(data$DR1TMOIS), lm_mos$residuals,500)
g_ns_mos <- plot_bins(invNorm(data$DR1TMOIS), ns_mos$residuals,500)
...
```{r ivn, echo=TRUE,message=FALSE,results = "asis"}
par(mfrow=c(1,2))
hist(data$DR1TBCAR)
hist(invNorm(data$DR1TBCAR))
```
