...
lm_base_str <- 'BPXDI1 ~ RIDAGEYR*RIAGENDR + BMXBMI'

ns_model <-
  'BPXDI1 ~ ns(RIDAGEYR, knots 
= seq(30, 80, by = 10)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10))) '
lm_base <- lm(as.formula(lm_base_str), data)
ns_base <- lm(as.formula(ns_model), data)


lm_mos_str <- 'DR1TMOIS ~ RIDAGEYR*RIAGENDR + BMXBMI'

ns_mos_str <-
  'BPXDI1 ~ ns(RIDAGEYR, knots 
= seq(30, 80, by = 10)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10)))'
lm_mos <- lm(as.formula(lm_mos_str), data)
ns_mos <- lm(as.formula(ns_mos_str), data)
...
ggpubr::ggarrange(g_age_calc_diff,g_bmi_calc_diff,
 ...
 mos_age_res <- list("Ldiff"=make_bins(x=data$RIDAGEYR,
                                       y=resid(lm_mos)-resid(lm_base),
                                       nbin=600),
                 "base"=make_bins(x=data$RIDAGEYR, 
                                    y=resid(ns_base),
                                   nbin=600),
                "mos"=make_bins(x=data$RIDAGEYR,   
                                    y=resid(lm_base),
                                    nbin=600)
                 )
plot_bins2(mos_age_res,xlab="Age(years)")
