...
age_res_diff <- plot_bins2(mos_age_res,xlab="Age(years)") +ylim(0.04,0.31)

bmi_res_diff <- plot_bins2(mos_bmi_res,xlab="BMI (kg/m²)")+ylim(0.04,0.31)
...
age_sug_diff <- plot_bins2(sug_age_res,xlab="Age (years)",title = "DR1TCALC")+ylim(0.04,0.31)

bmi_sug_diff <- plot_bins2(sug_bmi_res,xlab="BMI (kg/m²)",title = "DR1TCALC") +ylim(0.04,0.31)
...
- y-axis is binned difference residuals, and difference residuals are computed as $\left| R_1-R_2 \right|$ ,where $R_1$ is residuals of base model (eg, BPXDI1 ~ RIDAGEYR\*RIAGENDR + BMXBMI), and $R_2$ residuals of model with dietary variables(eg, BPXDI1 ~ RIDAGEYR\*RIAGENDR + BMXBMI+DR1TMOIS).


- In panel <em><strong> a) </strong></em>,  the yellow line show binned plot absolute of differences residuals of linear base model (`BPXDI1 ~ RIDAGEYR*RIAGENDR + BMXBMI`) and model linear model with DR1TMOIS(`BPXDI1 ~ RIDAGEYR*RIAGENDR + BMXBMI+DR1TCALC`). The blue line show binned plot absolute of differences residuals of spline base model (`BPXDI1 ~ ns(RIDAGEYR, knots 
= seq(20, 80, by = 10)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10)))`) and model spline model with DR1TMOIS(`BPXDI1 ~ ns(RIDAGEYR, knots 
= seq(20, 80, by = 10)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10)))+DR1TMOIS`). The values are binned according to BMI, and each bin contains about 600 points


- Similarly, panel <em><strong> b ) </strong></em>, the yellow line show binned plot absolute of differences residuals of linear base model (`BPXDI1 ~ RIDAGEYR*RIAGENDR + BMXBMI`) and model linear model with DR1TCALC(`BPXDI1 ~ RIDAGEYR*RIAGENDR + BMXBMI+DR1TCALC`). The blue line show binned plot absolute of differences residuals of spline base model (`BPXDI1 ~ ns(RIDAGEYR, knots 
= seq(20, 80, by = 10)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10)))`) and model spline model with DR1TCALC(`BPXDI1 ~ ns(RIDAGEYR, knots 
= seq(20, 80, by = 10)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10)))+DR1TCALC`). The values are binned according to BMI, and each bin contains about 600 points

- <em><strong> c) </strong></em> and <em><strong> d) </strong></em> are similar to <em><strong> a) </strong></em> and <em><strong> b) </strong></em>, except the data points are binned according to age.



```{r cook_lev, echo=TRUE,message=FALSE,results = "asis",dpi = 200}
ggpubr::ggarrange(bmi_res_diff,bmi_sug_diff,
                  age_res_diff,age_sug_diff,
                  common.legend = TRUE,
                  labels = c('a)','b)','c)','d)'),
                  legend="right")
```
...
age_sug_diff <- plot_bins2(sug_age_res,xlab="Age (years)",title = "DR1TCALC")

bmi_sug_diff <- plot_bins2(sug_bmi_res,xlab="BMI (kg/m²)",title = "DR1TCALC")
ggpubr::ggarrange(bmi_sug_diff,
                 age_sug_diff,
                 common.legend = TRUE)
```
