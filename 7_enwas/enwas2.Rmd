...
# drop diastolic. blood pressure and waist circumference for now because we are running systolic.

data <- subset(phs_data$data,select=-c(BPXDI1,BPXDI2,BMXWAIST))

data <- na.omit(data)

data$SYSTOLIC <- (data$BPXSY1+data$BPXSY1)/2
...
ns_str <-
  'SYSTOLIC ~ ns(RIDAGEYR, knots = seq(30, 80, by = 10), Boundary.knots=c(20,90)) * RIAGENDR + ns(BMXBMI,knots = c(seq(15, 45, by = 5),seq(45,65,by=10)),Boundary.knots=c(10,75)) + RIDRETH1 + DMDEDUC2+years'
