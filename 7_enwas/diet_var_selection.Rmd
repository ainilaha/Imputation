...
select_matrice <- function(df){
  select_df <- data.frame(
    ...
    # number of NAs
    na_ratio = round(sapply(df, function(x) sum(is.na(x)))/nrow(df),4),
...
Manually remove some the columns according to documents:
WTDRD1: Dietary day one sample weight
DR1EXMER: Interviewer ID code
DR1IFDCD:USDA food code

 eg. data$DRABF only contains 2
...
library(knitr)
diet_data <- diet_data[, -which(names(diet_data) %in% c( "WTDRD1","DR1EXMER","DR1IFDCD"))]

select_df <- select_matrice(diet_data)
kable(head(select_df))
select_vars <- select_df |> filter(std!=0)|>
  filter((phesant == 'binary' & rept_ratio < 0.95) | 
           (phesant=='continuous' & rept_ratio < 0.1 & zero_ratio < 0.1)|
           (phesant %in% c('ordered', 'unordered') & rept_ratio < 0.8)
                                               )
