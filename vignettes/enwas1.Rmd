...
forest_plot_mult(list(linear = lm_enwas$enwas_res,ns = ns_enwas$enwas_res),30)
...
library(kableExtra)
ns_enwas$qc_mtx |>
  dplyr::arrange(AIC)|> head(20)|>
  kbl() |> kable_classic_2(full_width = F)
```

### 6. ANOVA LRT and Vuong Test

```{r lrt, echo=TRUE,warning=F, out.width = '90%',dpi = 200}

ns_enwas$lrt_mtx |>
  dplyr::arrange(dplyr::desc(Deviance))|> head(20)|>
  kbl() |> kable_classic_2(full_width = F)
```
