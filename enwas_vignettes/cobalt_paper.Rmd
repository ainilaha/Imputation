...
data$DMDEDUC2 <- as.factor(nDEDUC)
 ...
 ##set up hypertension
 data$hypertension <- data$DIASTOLIC >= 90 | data$SYSTOLIC >= 140 |  bpq50A == 1
...
```{r model2,warning=FALSE,message=FALSE}
   base_logit <- glm(hypertension ~ RIDAGEYR + RIAGENDR + BMXBMI+DMDEDUC2+RIDRETH1 + sqrt(LBXBC0), data = data, family = "binomial")
   ...
   base_logit <- glm(hypertension ~ ns(RIDAGEYR,df=7) + RIAGENDR + ns(BMXBMI,df=7) + DMDEDUC2 + RIDRETH1, data = data2, family = "binomial")
