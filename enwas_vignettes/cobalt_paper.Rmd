...
comparability between analyses.
  FIXME: Laha, can you produce a table with the PHESANT output for these features?
  FIXME: Laha can you put some code in here to cut the continuous variables into 20 groups
so we can plot the risk of hypertension, per group?

```{r QA/QC, warnings=FALSE, message=FALSE}
  ## glm linear in the covariates
    base_logit <- glm(hypertension ~ RIDAGEYR + RIAGENDR + BMXBMI+DMDEDUC2, data = data, family = "binomial")

 ##spline covariates
  base_logit <- glm(hypertension ~ ns(RIDAGEYR,df=7) + RIAGENDR + ns(BMXBMI,df=7) + DMDEDUC2 + RIDRETH1, 
