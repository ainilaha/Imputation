...
```
## QA/QC
We now describe the process used to assess the base model.
We have chosen to adjust for age, sex, BMI, ethnicity and education level.
Initially we will fit simple natural splines, using automatic selection of the knots, but as
we progress we will likely chose a set of sensible knots for each of the exposures to support 
comparability between analyses.
  FIXME: Laha, can you produce a table with the PHESANT output for these features? (DONE)
  FIXME: Laha can you put some code in here to cut the continuous variables into 20 groups(DONE)
so we can plot the risk of hypertension, per group?
...
data = phs_dat$data
lm_logit <- glm(hypertension ~ RIDAGEYR + RIAGENDR + BMXBMI+DMDEDUC2, data = data, family = "binomial")
...
There is no much difference when plot residuals against age.
```{r}
library(arm)
layout(matrix(c(1,2,3,4), ncol=2, byrow=TRUE))
par(mar=c(4.0,4.0,2.5,2))
binnedplot(data$RIDAGEYR,residuals(lm_logit,type = "response"),nclass=20,xlab="Age",col.int="red")
binnedplot(data$RIDAGEYR,residuals(ns_logit,type = "response"),nclass=20,xlab="Age",col.int="blue")
binnedplot(data$BMXBMI,residuals(lm_logit,type = "response"),nclass=20,xlab="BMI",col.int="red")
binnedplot(data$BMXBMI,residuals(ns_logit,type = "response"),nclass=20,xlab="BMI",col.int="blue")

```
However, the following plot shows the spline model better than linear model.
```{r}

layout(matrix(c(1,2), ncol=2, byrow=TRUE))
par(mar=c(4.0,4.0,3.5,1))
binnedplot(fitted(lm_logit),residuals(lm_logit,type = "response"),col.int="red")
binnedplot(fitted(ns_logit),residuals(ns_logit,type = "response"),col.int="blue")
```
