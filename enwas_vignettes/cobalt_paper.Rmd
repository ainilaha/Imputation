...
```
The following plots show binned Hypertension data; each binned contains about 600 data points. Binned Data presents the proportion of the participants who reported hypertension. Linear and Spline present the fitted values (probabilities) from the GLM with linear terms and apply the natural spline function on continuous terms of the participants who have hypertension. Both panels a) and b) show that the GLM model with spline terms fits the data very well, whereas the GLM model only with linear terms performs poorly.
