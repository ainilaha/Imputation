...
Now suppose we wanted to find variables that had LDL (low density lipoproteins).  To do that we can use the `nhanesSearch` function. This searches through the master list of all questions, over all Questionaires and year. By default the *Description* field is searched. In the code below we find a variable names corresponding to LDLs and then find all the tables that contain a variable with that name.
```{r}
s1 = nhanesSearch("LDL")
...
datatable(LDLTabs)
```
 We have found a variable named `LDL` in a set of tables, but we need to find out more about these variables. Information about them is stored in the NHANES codebooks. We use the `nhanesCodebook` function to get that information. This function returns a list of values that tells you all about the variable. The `Target` tells you which participants were eligible to be asked. The `SasLabel` is the variable label, the `Description` is the actual text of the question (sort of, or a description of the lab value).  And the `Codebook` tells you which values the variable can take.  When you want to combine variables across study years you will need to be careful to ensure that the `Codebooks` are compatible.  NHANES has changed variables, added or removed possible answers, and done many other things as the questionnaires have evolved.  It is **not safe** to assume that an identical variable name will have an identical interpretation.
 
```{r}
  cb1 = nhanesCodebook(LDLTabs[1,2], LDLTabs[1,1])
  cb1
```
