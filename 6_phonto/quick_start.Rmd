---
title: "Quick Start"
author: "Laha Ale, Christopher Endres, Robert Gentleman, and Teresa Filshstein-Sonmez"
...
We can efficiently access NHANES data using the phonto::nhanes... functions, which are implemented at the SQL level. By simply knowing the name of the data file or table, we can quickly check for information. Let's take a body measure table as an example.
...
<<<<<<< HEAD
Comprehensive lists of NHANES variables are maintained for each data group. For example, the demographics variables are available at https://wwwn.cdc.gov/nchs/nhanes/search/variablelist.aspx?Component=Demographics. This section provide descriptions with examples of functions designed to facilitate searching the NHANES database.
...
#### 2.2 nhanesSearchTableNames
>>>>>>> 177bd192ddcbd954e91e4bb19fa624cf8d9d40b5
...
longer descriptive names.  For our Dockerized container no restricted data can be accessed (i.e. the RDC tables are excluded) and the `includerdc` option is not supported.

For example, we can search the blood pressure related tables by the following code. We choose "BPX" as the parameters based on the CDC table name conventions. The users can pass more sophistic search patterns according "LIKE string" in sql Server, more details can be found [here](https://learn.microsoft.com/en-us/sql/t-sql/language-elements/like-transact-sql?view=sql-server-ver16).

```{r}
res = nhanesSearchTableNames("BPX", details=TRUE)
datatable(res)
```
##FIXME: we need to rationalize this to not use long descriptive names in public
...
#### 1.2 nhanesSearch
 ...
 One of the main challenges in using the NHANES data set is finding the tables that contain the data you want to analyze. The `nhanesSearch` function allows the investigator to input search terms, match against the comprehensive variable descriptions, and retrieve the list of matching variables. In this section we outline some of the basic methods you can use to do the searching. The returned value can be put into a searchable data table that you can use to further filter, sort and search for specific variables. 
 
=======
#### 2.3 Searching for Variables
 One of the main challenges in using the NHANES data set is finding the tables that contain 
 the data you want to analyze.  In this section we outline some of the basic methods you can use to do the searching. The returned value can be put into a searchable data table that you can use to further filter, sort and search for specific variables.
>>>>>>> 177bd192ddcbd954e91e4bb19fa624cf8d9d40b5
```{r}
...
Now suppose we wanted to find variables names that contain information about (low density lipoproteins).  To do that one can first use the `nhanesSearch` function to retrieve a master list of any and all questions matching this search term in it's *Description*. One can then use the output of this search to identify variable names of interest. The variable name can then be search for using the `nhanesSearchVarName` function.  In the code below we find a variable names corresponding to LDLs and then find all the tables that contain a variable with that name.

```{r}
s1 = nhanesSearch("LDL")
##from this one can obtain Variable names
s1[1,c(2:3)]
...
The `unionQuery()` function is designed to aggregate data across the years for a fixed Questionaire. Thus we are assuming all the inputs have essentially the same columns and that all variables are measuring the same thing across years. This function then aggregates by appending rows and returns the results as a data frame. If there are columns that are unique to one (or a few) questionaires then these will be filled in as NA in any questionaires that don't have that column.  Currently there is no checking of the Codebooks across years, analysts will need to do that themselves. For example, the blood pressure tables in CDC are BPX, BPX_B,...BPX_J from years 1999-2000 to 2017-2018. The users can aggregate some or all of the data.  And while have shown this example without carefully checking the Codebook, we encourage you to take the time and ensure that Codebooks are compatible.
...
The real factor levels can be set with `nhanesTranslate()` function. This function uses
the data in the NHANES Codebook for a given variable and uses that to translate categorical variables from their integer codes into the appropriate levels.
##FIXME: need to figure out if these are always coming back as character vectors - which seems best, and those can then be transformed into factors (ordered if desired).  Compare our implementation to that of nhanesA
...
##FIXME: Laha - somewhere you should have an explicit definition of what is in the code - what makes something continuous - what makes it discrete, what makes it ordered? In the first pass, all the categorical values are discrete integers - that does not make them ordered - you have to look at the actual levels - and we need to explain how our users will do that.
