...
The [National Health and Nutrition Examination Survey （NHANES)](https://www.cdc.gov/nchs/nhanes/index.htm) datasets are collected from the Centers for Disease Control and Prevention in the USA, including demographics, dietary, laboratory, examination, and questionnaire data. 
...
##FIXME: can you describe the SQLserver technology that Nathan is using and provide some URL (or other) references to how to query that. Point out that we have some R wrappers (along the lines of nhanesA functions) but that the user might benefit from using SQL directly. They will need to get pointed to a reference manual.
...
##FIXME - we need an example of the longer descriptive names
