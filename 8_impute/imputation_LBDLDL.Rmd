...
data = phonto::nhanesTranslate('DEMO_I', c('RIAGENDR', 'RIDRETH1'), data=data) # convert gender and ethnicity to factors
```
...
```{r}
data = data[,c("SEQN","RIDAGEYR","RIAGENDR" , "RIDRETH1" , "DMDEDUC2" ,"LBDLDL" ,"diabetes" ,"SYSTOLIC" ,"smoking")]
data = na.omit(data)

# split training and testing data set
test_ix = sample(x = 1:nrow(data), size = 1500) # about 10%
train_ix = sample(x = setdiff(1:nrow(data), test_ix), size=20000)
...
```{r}
lm_reg = lm(formula = LBDLDL ~ ns(RIDAGEYR, knots = seq(30, 80, by = 10), Boundary.knots=c(20,90))*RIAGENDR + RIDRETH1+DMDEDUC2+smoking+SYSTOLIC, data = train_data)
