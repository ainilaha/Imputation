...
lm_reg = lm(formula = LBDLDL ~ ns(RIDAGEYR) + RIAGENDR + RIDRETH1, data = data)
```

### add missingness 

```{r}
test_ix = sample(x = 1:nrow(data), size = 2000) # about 10%
train_ix = sample(x = setdiff(1:nrow(data), test_ix), 20000)

train_data = data[train_ix, ]
test_data = data[test_ix, ]

miss_data = data
miss_data[test_ix,"LBDLDL"]=NA
```
