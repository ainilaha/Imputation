...
tBPQ = unionQuery(BPQtabs, cols = c('BPQ080','BPQ100D'))
...
bpq_df = unionQuery(bpq_tables,cols = c('BPQ050A','BPQ020','BPQ060','BPQ080','BPQ100D'))
...
bpx_df$hypertension <- bpx_df$DIASTOLIC >= 90 | bpx_df$SYSTOLIC >= 140 |  bpx_df$BPQ050A==1
...
```{r}
mod = lm(formula = LBDLDL ~ ns(RIDAGEYR, df=7) + cholMeds + RIAGENDR, data=data[data$RIDAGEYR>50,]) # Multiple R-squared:  0.02315,	Adjusted R-squared:  0.02306 
mod = lm(formula = LBDLDL ~ ns(BMXBMI, df=7) + cholMeds + RIAGENDR, data=data[data$RIDAGEYR>50,])  # Multiple R-squared:  0.001888,	Adjusted R-squared:  0.001795 
mod = lm(formula = LBDLDL ~ DMDEDUC2 + cholMeds, data=data[data$RIDAGEYR>50,])  # Multiple R-squared:  0.00026,	Adjusted R-squared:  7.686e-05 
mod = lm(formula = LBDLDL ~ ns(BMXWT, df=7) + cholMeds, data=data[data$RIDAGEYR>50,]) # Multiple R-squared:  0.00026,	Adjusted R-squared:  7.686e-05 
